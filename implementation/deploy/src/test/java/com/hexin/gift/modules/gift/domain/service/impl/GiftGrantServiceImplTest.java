...
class GiftGrantServiceImplTest {
    ...
    @Mock
    private PortfolioGoodsApi portfolioGoodsApi;
    ...
    void setUp() {
        ...
        giftGrantService = new GiftGrantServiceImpl(activityGiftRightsApi, portfolioGoodsApi, packageGoodsApi, fixedClock);
    }

    @Test
    void grantBatch_shouldReturnTrueWhenSuccess() {
        GoodsBaseVO good = new GoodsBaseVO(1L, "portfolio", "PORTFOLIO", 88);
        when(portfolioGoodsApi.getGoodsIdById(1L)).thenReturn(9001L);
        ...
        verify(activityGiftRightsApi).addactivitygiftrights(eq(9001L), attrCaptor.capture(), eq(1001), sourceCaptor.capture());
    ...
    void grantBatch_shouldReturnFalseWhenException() {
        ...
        doNothing().when(activityGiftRightsApi)
                .addactivitygiftrights(eq(9001L), any(SellInfoAttr.class), eq(1001), anyString());
        doThrow(new RuntimeException("fail"))
                .when(activityGiftRightsApi)
                .addactivitygiftrights(eq(9001L), any(SellInfoAttr.class), eq(1002), anyString());
        ...
        verify(activityGiftRightsApi).addactivitygiftrights(eq(9001L), any(SellInfoAttr.class), eq(1001),
                eq("Advisor 88 granted product {1, PORTFOLIO} to user 1001 on 2024-01-01 08:30:15"));
        verify(activityGiftRightsApi).addactivitygiftrights(eq(9001L), any(SellInfoAttr.class), eq(1002),
